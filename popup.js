...
function loadNPCData() {
  ...
  fetch(chrome.runtime.getURL('npcData.json'))
...
function generateNPC() {
  chrome.storage.sync.get(null, settings => {
      ...
      chrome.storage.sync.set({ lastNPC: npc });
  });
}
...
function loadLastNPC() {
  chrome.storage.sync.get(['lastNPC', 'showAge', 'showGender', 'showRace', 'showHairstyle', 'showUniqueFeature', 'showHeight', 'showBuild'], data => {
      if (data.lastNPC) {
...
function generateFantasyName(prefixes, suffixes) {
  ...
  function isHardConsonantPair(prefixEnd, suffixStart) {
      const hardPairs = [
          ...
          "ml", "mp", "mq", "mv", "mx", "nb", "nc", "nv", "nk", "nf", 
          "nj", "nk", "nl", "np", "nq", "ns", "nj", "nx", "pv", "pz", 
